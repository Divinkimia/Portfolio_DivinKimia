/**
 * Page Veilles — parcours (sidebar, accordéon, contenu dynamique)
 * + effet galaxie & navbar alignés sur le reste du portfolio
 */
(function () {
  'use strict';
  ...
  /** Masque la navbar du site en plein écran (plus d’espace lecture) ; réaffiche à la sortie (Échap, bouton…). */
  ...
  function selectLesson(index) {
    ...
    document.title = 'Divin Kimia | ' + L.title + ' — OpenClaw';
    if (lead) lead.textContent = L.lead;

    const objTitle = document.getElementById('veilleObjectifsTitle');
    if (objTitle) objTitle.textContent = L.bullets && L.bullets.length ? 'Points clés' : 'Synthèse';
